...
class Command(BaseCommand):
    help = "Update local and internet-based ongoing and upcoming events cache. Note that cache has to be set up and\
    ...
    def handle(self, *args, **options):
        ...
        inet_events = Event.objects.filter(location__id__endswith='internet',
                                           end_time__gte=datetime.utcnow().replace(tzinfo=pytz.utc),
                                           deleted=False,
                                           ).values_list('id', 'name', 'description', 'short_description',
                                                         'name_en', 'description_en', 'short_description_en',
